...
public class GameManager : MonoBehaviour
{
    ...
    public Sprite[] GO2Sprites;
    public Sprite[] GO4Sprites;
    public Sprite[] GO8Sprites;
    public Sprite[] GO16Sprites;
    public Sprite[] GO32Sprites;
    public Sprite[] GO64Sprites;
    ...
    private void CreateNewItem(int value = 2, int? row = null, int? column = null)
    {
        ...
        if (row == null && column == null)
        {
            ...
            // matrix2.GetRandomRowColumn(out randomRow, out randomColumn);
            // matrix3.GetRandomRowColumn(out randomRow, out randomColumn);
            // matrix4.GetRandomRowColumn(out randomRow, out randomColumn);
        }
        else
        {
            randomRow = row.Value;
        ...
        //newItem.GO.transform.DoScale(Globals.AnimationDuration, new Vector3(1.0f, 1.0f, 1.0f));
        newItem.GO.transform.DOScale(new Vector3(1.0f, 1.0f, 1.0f),Globals.AnimationDuration);

        matrix[randomRow, randomColumn] = newItem;
        // matrix2[randomRow, randomColumn] = newItem;
        // matrix3[randomRow, randomColumn] = newItem;
        // matrix4[randomRow, randomColumn] = newItem;
    }
    ...
    IEnumerator AnimateItems(IEnumerable<ItemMovementDetails> movementDetails)
    {
        ...
        foreach (var item in movementDetails)
        {
            ...
            //move it there
            var tween =
                item.GOToAnimatePosition.transform.DOMove(newGoPosition,Globals.AnimationDuration,false);
            ...
            if (item.GOToAnimateScale != null)
            {
                ...
                newGO.transform.localScale = new Vector3(0.01f, 0.01f, 0.01f);
                newGO.transform.DOScale(1.0f,Globals.AnimationDuration);
                ...
                var moveTween = item.GOToAnimateScale.transform.DOMove(newGoPosition,Globals.AnimationDuration,false);
                //then, the scale one
                var scaleTween = item.GOToAnimateScale.transform.DOScale(0.1f,Globals.AnimationDuration);

                var chain = DOTween.Sequence();
                chain.Append(moveTween).AppendInterval(Globals.AnimationDuration).Append(scaleTween);
                chain.Play();
